<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows >= 1) {
      ...
      $t_cookie = setcookie("tenant",$row['first_name']) ?? $user;
      setcookie("user", $user);
      $output=true;
      header('Content-Type: application/json');
    ...
    else
    {
      ...
      if ($result->num_rows >= 1) {
        ...
        if ($status == "Rejected") {
          ...
          $error = json_decode(file_get_contents('error.json'), true)['1'];
          header('Content-Type: application/json');
          echo json_encode($error);
          exit();
        ...
        $error = json_decode(file_get_contents('error.json'), true)['3'];
        header('Content-Type: application/json');
        echo json_encode($error);
        exit();
