...
<body>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
      $rname = mysqli_real_escape_string($conn,  $user);
      ...
      $sql = "INSERT INTO ams_complaint (Username, ComplaintRegarding, `Description`, current_status)
        VALUES ('$rname','$complaintReg', '$complaintDes', 'Pending')";
  ...
  <?php
    ...
    // Handle form submission
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // Code for handling form submission remains unchanged
    }
    ?>
    ...
    <div class="card_containerz">
      ...
      $sql = "SELECT * FROM ams_complaint where Username = '$user' ORDER BY id DESC";
      ...
      if ($result->num_rows > 0) {
          ...
          while ($row = $result->fetch_assoc()) {
              ...
              if (strlen($description) > 20) {
                  echo '<div class="description-content show-more-content" style="max-height: 100px;">';
                  echo ucwords(substr($description, 0, 20));
                  echo '<span class="hidden-full-description" style="display:none;">' . $description . '</span>';
                  echo '<span class="show-more-btn" onclick="showMoreLess(this)">Show More</span>';
                  echo '</div>';
              } else {
      ...
      showMoreButtons.forEach(button => {
        button.addEventListener('click', function() {
          ...
          if (content.classList.contains('show-more-content')) {
              ...
              content.querySelector('.hidden-full-description').style.display = 'inline';
            } else {
              this.textContent = 'Show Less';
              content.querySelector('.hidden-full-description').style.display = 'none';
            }
          });
        });
      });
