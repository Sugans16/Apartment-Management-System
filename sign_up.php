    ...
    if ($result) {
        if ($result->num_rows >= 1) {
            ...
            if ($row['status'] == "Accepted") {
                $error = json_decode(file_get_contents('error.json'), true)['6'];
          ...
          exit();
                ...
                $error = json_decode(file_get_contents('error.json'), true)['8'];
                header('Content-Type: application/json');
                echo json_encode($error);
                
            }
            else {
                $error = json_decode(file_get_contents('error.json'), true)['2'];
          header('Content-Type: application/json');
          echo json_encode($error);
          exit();
            }
            ...
            if ($con->query($insertQuery)) {
                $error = json_decode(file_get_contents('error.json'), true)['7'];
          header('Content-Type: application/json');
          echo json_encode($error);
          exit();
            } else {
